...
namespace ProgrammersBlog.Services.Concrete
{
    public class CategoryManager : ICategoryService
    {
        ...
        public async Task<IResult> Delete(int categoryId,string modifiedByName)
        {
            ...
            return new Result(ResultStatus.Error, "Böyler Bir Kategori Bulunamadı.");
        }
        ...
        public async Task<IDataResult<Category>> Get(int id)
        {
            ...
            return new DataResult<Category>(ResultStatus.Error, "Böyler bir kategori bulunamadı", data: null);
        }

        public async Task<IDataResult<IList<Category>>> GetAll()
        {
            var categories = await _unitOfWork.Categories.GetAll(null, c => c.Articles);
        ...
        {
            var categories=await _unitOfWork.Categories.GetAll(c=>!c.IsDeleted,c=> c.Articles);
            if (categories.Count > -1)
            {
                return new DataResult<IList<Category>>(ResultStatus.Succes, categories);
            }
        ...
        public async Task<IResult> Update(CategoryUpdateDto categoryUpdateDto, string modifiedByName)
        {
            ...
            if (category!=null)
            {
                ...
                category.ModifiedDate = DateTime.Now;
               await _unitOfWork.Categories.UpdateAsync(category).ContinueWith(t=>_unitOfWork.SaveAsync());
                return new Result(ResultStatus.Succes, $"{categoryUpdateDto.Name} başarıyla güncellenmiştir.");
            }
            return new Result(ResultStatus.Error, "Böyle bir kategori bulunamadı.");
        }
    }
